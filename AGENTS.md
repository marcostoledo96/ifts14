...
- En ciclos SDD verificados (post `sdd-verify` PASS), OpenCode PUEDE ejecutar `git add` + `git commit` + `git push` (a la rama de trabajo actual, nunca a `main`) SOLO cuando Matías o Marcos lo aprueben explícitamente en el mismo turno del chat, con el mensaje y comando exactos propuestos. **Pre-commit safety (diff-confirmation gate)**: previo al `git add`, OpenCode DEBE correr `git status --short` y `git diff --name-only`, presentar el resultado y esperar confirmación de que el diff es correcto. **Pre-push safety**: previo al `git push`, OpenCode DEBE correr `git log origin/<rama>..<rama> --oneline` y `git diff origin/<rama>..<rama> --stat`, presentar el resultado y esperar confirmación. `git switch`, `git checkout`, `git branch`, `git switch -c` y `git checkout -b` quedan permitidos solo para navegación o creación no destructiva de ramas, con árbol limpio, rama fuente explícita y actualizada, y aprobación explícita de Matías o Marcos. PR solo se puede crear/abrir con aprobación explícita. Permanecen PROHIBIDOS `git merge`, `git rebase`, `git push` a `main` y merge de PR.
- No subir secretos.
