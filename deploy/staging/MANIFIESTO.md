...
|---|---|---|
| `apps/frontend-angular/dist/frontend-angular/browser/` | `certificados_staging/` | Build `production-staging`. Contiene `index.html`, assets y JS bundles con `baseHref /certificados_staging/`. |
...
| `apps/backend-php/composer.lock` | `certificados_staging/api/composer.lock` | Para `composer install` en hosting o para regenerar `vendor/` local. |
...
| `dist/` (carpeta raíz del repo) | Artefactos de build previos. Solo se copia el `dist/frontend-angular/browser/` del build `production-staging` reciente. |
