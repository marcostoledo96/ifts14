...
const forbiddenField = /(?:^|[^\w])["']?(?:dni|token|legajo|matr[ií]cula)["']?\s*:/i;
const forbiddenLiteral = /(?:^|[^\w])["'](?:dni|token|legajo|matr[ií]cula)["']/i;
...
describe('students no network or storage', () => {
  ...
  it('rechaza campos sensibles sin confundir dniMostrar ni email opcional del DTO', () => {
    const source = [
      JSON.stringify(seed),
      seed.toString(),
    ...
    expect('{ dniMostrar: "20111222", email: "persona.uno@example.invalid", tieneEmail: true }').not.toMatch(forbiddenField);
    for (const sample of ['dni: "123"', 'token:', 'legajo:', 'matrícula:', '"legajo"', '123e4567-e89b-12d3-a456-426614174000']) {
