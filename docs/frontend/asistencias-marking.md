...
Ciclo `auto-estado-fecha-curso`: el mock de `ATTENDANCE_SOURCE` aplica la misma regla que el backend al marcar presentes (`realizada` solo con ≥1 presente y `fecha < hoy` AR; si no, `programada`; `cancelada` rechazada). La fuente HTTP hereda `fechaEstado` del API. Archive: `openspec/changes/archive/2026-07-20-auto-estado-fecha-curso/`.
