...
- Revisar el inventario v0 al iniciar cada ciclo para detectar cambios en `muestra_pagina/README.md`.
...
- Inventar pantallas o flujos para prompts 11-22 sin spec y diseño aprobados.
- Implementar PDF, QR, revocación, envío de certificaciones o configuración institucional sin spec previa.
- Decidir cambios en `angular.json`, `package.json` o estilos globales sin coordinar con Marcos.
- Ejecutar `git merge`, `git rebase`, push a `main` o merge de PR.

## Fuentes de verdad

La síntesis enlaza las 8 fuentes vigentes en lugar de duplicarlas (`docs/AGENTS.md:11`).
...
| `muestra_pagina/README.md` | Estado actual de la referencia visual: 7 pantallas disponibles, 12 pendientes. |
| `muestra_pagina/AGENTS.md` | Reglas de uso de la referencia visual: extraer intención, no copiar literalmente. |
...
`muestra_pagina/` contiene la referencia v0 exportada de Next.js/React.
Según `muestra_pagina/README.md` hay 7 pantallas disponibles (prompts 4-10) y 12 pendientes (prompts 11-22).
La regla original "si está vacía, bloquea UI final" no aplica literalmente hoy; la regla efectiva es "no inventar pantallas para flujos sin diseño aprobado".
Esto significa que Matías puede trabajar sobre los prompts 4-10, pero debe esperar spec y diseño aprobados antes de implementar los flujos 11-22.

Las 7 pantallas disponibles son: validación pública válida, estados públicos no exitosos, dashboard administrativo, login administrativo, crear/editar curso, registrar asistencias y emitir certificación directa.
Los 12 flujos pendientes incluyen detalle de certificación, vista previa PDF, listados de cursos, alumnos y certificaciones, revocación, envío, auditoría y configuración institucional.
...
De `AGENTS.md:21` (política Git vigente):

- No ejecutar `git merge`, `git rebase`, push a `main` ni merge de PR.
...
- [muestra_pagina/README.md](../../muestra_pagina/README.md): declara el estado actual de la referencia visual (7 pantallas listas, 12 pendientes) y su uso permitido. Actualiza el supuesto de si la carpeta está vacía.

- [muestra_pagina/AGENTS.md](../../muestra_pagina/AGENTS.md): reglas de uso de la referencia visual: extraer intención, no copiar React/Next literalmente y no implementar si solo hay README y AGENTS. Limita cómo se usa la referencia.
