# Delta para frontend-api-readiness
...
(Previously: la frontera solo declaraba la posibilidad futura de reemplazo; ahora la conmutación local mock→API real es un comportamiento exigido y verificable.)
...
- **Entonces** el consumidor de la pantalla DEBE permanecer agnótico a la fuente activa.
...
(Previously: el mapeo se declaraba para respuestas futuras; ahora debe verificarse contra la API PHP real en el smoke local.)
