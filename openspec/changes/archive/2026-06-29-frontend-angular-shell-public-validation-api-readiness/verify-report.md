...
| Runtime smoke | `npm start -- --configuration development --host 127.0.0.1 --port 4200 --serve-path /certificados/` + Playwright | ✅ `/certificados/` renders the non-validating landing page; `validar/demo-valido` renders a valid certificate; `revocado|expirado|inexistente` render the same not-verifiable text; `error-tecnico` renders the separate safe message; browser warnings/errors: 0 |
...
|---|---|---|---|
| Shell Angular bajo `/certificados/` | Entrada al módulo público | Playwright opened `/certificados/`; app rendered the shell and a non-validating landing page without real data or API calls. | ✅ COMPLIANT |
